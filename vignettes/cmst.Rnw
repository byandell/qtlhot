...
%application of the causal model selection tests (Chaibub Neto et al. 2012), implemented in the \texttt{cmst} R package, to a few simulated toy examples, and

\section{Basic functionality}

Here, we illustrate the basic functionality of the \texttt{R/qtlcmst} package in a toy simulated example.
...
There are several other functions involved in simulation and in data analysis that are not well documented yet. They are in fact hidden behind the NAMESPACE. See for instance the \texttt{R/qtlyeast} for some analysis routines.
